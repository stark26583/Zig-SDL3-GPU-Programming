        ...
        while (sdl3.c.SDL_PollEvent(&event)) {
            ...
            switch (event.type) {
                ...
                sdl3.c.SDL_EVENT_KEY_DOWN => {
                    ...
                    if (scancode == sdl3.c.SDL_SCANCODE_SPACE) {
                        Model.paused = !Model.paused;
                        std.debug.print("Mass space\n", .{});
                    }
                    if (scancode == sdl3.c.SDL_SCANCODE_ESCAPE) {
                        return false;
                    }
                    if (scancode == sdl3.c.SDL_SCANCODE_W) {
                        Camera.w_pressed = true;
                    }
                    if (scancode == sdl3.c.SDL_SCANCODE_A) {
                        Camera.a_pressed = true;
                    }
                    if (scancode == sdl3.c.SDL_SCANCODE_S) {
                        Camera.s_pressed = true;
                    }
                    if (scancode == sdl3.c.SDL_SCANCODE_D) {
